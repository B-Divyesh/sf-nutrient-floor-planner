...
function routeCopy(kind: 'privacy' | 'terms') {
  const privacy = `<h1 tabindex="-1">Your meal plan stays on this device.</h1><p>Nutrient Floor stores foods, targets, and meals in your browser’s local database. We do not run analytics or send your nutrition data to us.</p><h2>What is stored</h2><p>Your plan stays on this device until you export it, import another plan, or clear browser data. Demo data uses a separate local space and is discarded when you leave.</p><h2>Contact</h2><p>For product questions, email <a href="mailto:hello@sociobot.in">hello@sociobot.in</a>.</p>`;
  const terms = `<h1 tabindex="-1">Use Nutrient Floor for personal meal planning.</h1><p>Nutrient Floor compares food values you enter with targets you choose. It is not medical advice or a nutrition diagnosis.</p><h2>Your data</h2><p>Check food labels and sources before relying on a value. Keep an export if your plan matters to you.</p><h2>No warranty</h2><p>The app is provided as-is, to the extent allowed by law.</p>`;
  return kind === 'privacy' ? privacy : terms;
}

function landing() {
  return `<main id="main"><section class="hero" aria-labelledby="hero-title"><div class="hero-copy"><p class="eyebrow">A PRIVATE MEAL PLANNING SHEET</p><h1 id="hero-title" tabindex="-1">Plan meals that meet your nutrient targets.</h1><p class="lede">For home cooks who want enough fibre or protein without logging every calorie.</p><div class="hero-actions"><a class="button primary" href="/demo" data-route>Try it with sample data</a><span>Loads a seven-food plan.</span></div><ul class="facts"><li>Stored on this device</li><li>Works offline after setup</li><li>Export or import your plan</li></ul></div><figure class="hero-art"><img src="/assets/hero.webp" width="1200" height="800" fetchpriority="high" decoding="async" alt="Ingredients arranged across a blue kitchen planning sheet." /><figcaption>Original generated illustration; food values are entered by you.</figcaption></figure></section><section class="live-preview ruled"><div><p class="eyebrow">THE QUESTION IT ANSWERS</p><h2>Will this small menu clear my floor?</h2><p>Pick a few targets, save familiar foods, and place meal portions on a week.</p></div><div class="mini-board"><span>FIBRE</span><b>33.5 g</b><i>above 30 g floor</i><span>PROTEIN</span><b>75.5 g</b><i>above 75 g floor</i></div></section><section class="how" aria-labelledby="how-title"><h2 id="how-title">Plan a week in three steps</h2><ol><li><b>01 / Set a target</b><span>Choose a floor or limit in grams.</span></li><li><b>02 / Save trusted foods</b><span>Copy values from a label or source.</span></li><li><b>03 / Place meals</b><span>See gaps before you cook.</span></li></ol></section><section class="plain-note"><h2>What this does not do</h2><p>It does not count calories, diagnose health conditions, or upload a food diary.</p></section></main>`;
}
...
function page() {
  if (!knownRoutes.has(activeRoute)) return `${header()}${notFound()}${footer()}`;
  if (activeRoute === '/privacy' || activeRoute === '/terms') return `${header()}<main id="main" class="legal"><article>${routeCopy(activeRoute.slice(1) as 'privacy' | 'terms')}</article></main>${footer()}`;
  if (activeRoute === '/' && !demo) return `${header()}${landing()}${footer()}`;
  return `${header()}${demoBanner()}${planner()}${footer()}${dialogMarkup()}<div class="toast" role="status" aria-live="polite">${e(notice)}${waitingWorker ? '<button class="text-button" data-action="apply-update">Update now</button>' : ''}</div>`;
}
function openDialog() {
...
function navigate(path: string) { history.pushState({}, '', path); demo = path === '/demo'; focusRouteHeading = true; window.scrollTo(0, 0); void load(); }
...
document.addEventListener('click', async event => {
  ...
  if (route) { event.preventDefault(); navigate(route.pathname); return; }
  ...
  if (action === 'close-dialog') { dialog = null; render(); return; }
  if (action === 'show-food') { dialog = { kind: 'food' }; render(); return; }
  if (action === 'show-target') { if (plan.targets.length >= 5) { notice = 'You can save up to five targets.'; render(); } else { dialog = { kind: 'target' }; render(); } return; }
  if (action === 'new-meal') { dialog = { kind: 'meal', day: Number(el.dataset.day || 0) }; render(); return; }
  if (action === 'edit-meal') { const meal = plan.meals.find(m => m.id === id); if (meal) { dialog = { kind: 'meal', id, day: meal.day }; render(); } return; }
  if (action?.startsWith('ask-delete-')) { dialog = { kind: 'confirm', subject: action.slice('ask-delete-'.length) as 'food' | 'target' | 'meal', id }; render(); return; }
  if (action === 'reset-demo') { await clearPlan('demo:plan'); plan = samplePlan(); await save(); notice = 'Sample plan reset.'; render(); return; }
  if (action === 'start-real') { await clearPlan('demo:plan'); demo = false; history.replaceState({}, '', '/plan'); focusRouteHeading = true; notice = 'Your private plan is ready.'; await load(); return; }
...
document.addEventListener('close', event => { if ((event.target as HTMLElement).matches('dialog[data-dialog]') && dialog) { dialog = null; render(); } }, true);
document.addEventListener('submit', async event => {
  ...
  if (formName === 'confirm-delete' && dialog?.kind === 'confirm') {
    ...
    dialog = null; await save(); render(); return;
  }
  if (formName === 'food') {
...
document.addEventListener('change', async event => {
  ...
  try {
    ...
    plan = incoming; await save(); notice = 'Plan imported.';
  } catch { notice = 'That file is not a valid Nutrient Floor plan. Choose an exported JSON file.'; }
  render();
});
window.addEventListener('popstate', () => { demo = location.pathname === '/demo' || new URLSearchParams(location.search).get('demo') === '1'; focusRouteHeading = true; void load(); });
